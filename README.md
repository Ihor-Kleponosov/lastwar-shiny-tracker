...
- Preset JSON transfer: choose one or more saved presets and download their unchanged `id`, `name`, and `enabledServerIds` data in a versioned portable JSON file; Import validates that format and lets you choose which presets to add as new copies
...
From the Presets page, Export lets you select saved presets and download a versioned JSON object with `version: 1` and a `presets` array containing their existing objects without changing their structure. Import validates that same JSON format before allowing you to select presets. Imported presets are always added as new copies with unique IDs and conflict-free names.
