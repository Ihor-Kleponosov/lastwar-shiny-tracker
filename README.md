...
- Full-screen preset configuration with search, inclusive range filtering, flat and grouped list views, bulk selection and clearing, draft saving, and unsaved-change confirmation
...
- Preset JSON export: choose one or more saved presets and download their unchanged `id`, `name`, and `enabledServerIds` data in a versioned portable JSON file; the adjacent Import control is reserved for a future release
- Each selected server-list card also provides a PNG export action with its preset preselected; only the export month can be changed
...
From the Presets page, Export lets you select saved presets and download a versioned JSON object with `version: 1` and a `presets` array containing their existing objects without changing their structure. Import is displayed as a placeholder and is not available yet.
